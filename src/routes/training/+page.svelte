<script lang="ts">
	...
	const fallbackSessions = [
		...
		{ id: '2', day_of_week: 'Thursday', start_time: '17:30', end_time: '19:30', location: 'Saliña Terrain, Willemstad', category: 'Senior Men & Women', notes: null, translations: {} },
			...
			<div class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
				<div class="flex flex-wrap items-start justify-between gap-4">
					<div>
						...
						{#if session.notes}<p class="mt-2 text-sm italic text-gray-500">{t(session, locale, 'notes')}</p>{/if}
					</div>
