...
<svelte:head><title>{m.news_page_title()} – Curaçao Rugby Federation</title></svelte:head>

<div class="bg-crf-blue py-16 text-center text-white">
	<h1 class="text-4xl font-extrabold sm:text-5xl">{m.news_page_title()}</h1>
	<p class="mt-2 text-white/70">{m.news_page_subtitle()}</p>
</div>
...
<div class="mx-auto max-w-4xl px-4 py-16 sm:px-6 lg:px-8">
	<div class="mb-10 rounded-2xl bg-gray-50 p-6 text-center">
		...
		<div class="flex flex-wrap justify-center gap-4">
			...
			<a href={SOCIAL.instagram} target="_blank" rel="noopener noreferrer" class="flex items-center gap-2 rounded-full bg-gradient-to-r from-purple-500 via-pink-500 to-orange-400 px-5 py-2 text-sm font-semibold text-white hover:opacity-90 transition">
			...
			<a href={SOCIAL.youtube} target="_blank" rel="noopener noreferrer" class="flex items-center gap-2 rounded-full bg-[#ff0000] px-5 py-2 text-sm font-semibold text-white hover:opacity-90 transition">
	...
	{#if data.posts.length === 0}
		<div class="rounded-2xl bg-gray-50 p-12 text-center">
			...
			<h2 class="mb-2 text-xl font-bold text-gray-700">{m.news_empty_title()}</h2>
			<p class="text-gray-500">{m.news_empty_body()}</p>
		</div>
	{:else}
